...
def convert_csv_to_json():
    """Convert CSV to JSON if it exists and delete old JSON before rewriting."""
    if not os.path.exists(INITIAL_CSV):
        raise FileNotFoundError(f"CSV file not found at {INITIAL_CSV}.")

    # Delete old JSON before rewriting
...
def preprocess_initial_dataset():
    ...
    try:
        ...
        return jsonify({
            ...
            "cleaned_data_preview": df.head(5).to_dict(orient='records'),
            "total_rows": len(df),
...
def process_user_input():
    ...
    try:
        ...
        return jsonify({
            ...
            "processed_data_preview": df.head(5).to_dict(orient='records'),
            "total_rows": len(df),
